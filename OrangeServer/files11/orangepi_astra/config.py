"""
Configuração central do ASTRA SPECTRE (versão Orange Pi).

PINOS: já confirmados via 'sudo gpio readall' na sua Orange Pi 4A
(ver tabela no README.md). Se remontar a fiação em pinos diferentes,
rode 'sudo gpio readall' de novo e atualize os valores abaixo.
"""

# ================= PINOS (wPi) =================
# Confirmados via 'sudo gpio readall' - ver README para o mapeamento
# físico completo.
PINOS = {
    # LEDs
    "LED_VERDE":   6,   # físico 12 (PB05) - já testado e funcionando
    "LED_AMARELO": 9,   # físico 16 (PI13)
    "LED_AZUL":    10,  # físico 18 (PI14)
    "LED_VERMELHO": 14, # físico 23 (SCLK.1) - alerta de não aderência

    # Botões (INPUT_PULLUP - pressionado = nível baixo, contato com GND)
    "BTN_VERDE":   19,  # físico 29 (PB03)
    "BTN_AMARELO": 20,  # físico 31 (PB11)
    "BTN_AZUL":    22,  # físico 33 (PWM13/268, usado aqui só como GPIO)
    "BTN_PRETO":   23,  # físico 35 (PB06)

    # Buzzer ativo (liga/desliga simples)
    "BUZZER": 25,       # físico 37 (PB12)
}

# ================= I2C (OLED) =================
# Deixe False por enquanto - liga quando for conectar o display.
OLED_HABILITADO = False

# Endereço padrão do SSD1306. Se `i2cdetect` mostrar outro endereço
# (ex: 0x3D), troque aqui.
OLED_I2C_ADDRESS = 0x3C
OLED_WIDTH = 128
OLED_HEIGHT = 64
# Deixe None para autodetectar o barramento I2C (recomendado).
# Se quiser forçar um barramento específico (ex: /dev/i2c-3), coloque 3.
OLED_I2C_BUS = None

# ================= IDENTIDADE DESTE DISPOSITIVO =================
# Cada Orange Pi (cada Helios) precisa de um ID único quando houver mais
# de uma unidade na mesma rede/broker MQTT - senão os tópicos colidem e
# o dashboard mistura o status de pacientes diferentes.
#
# Pode trocar aqui direto, ou setar a variável de ambiente ASTRA_DEVICE_ID
# na hora de instalar em cada unidade (assim não precisa editar código
# por unidade):
#   ASTRA_DEVICE_ID=helios-joao-silva sudo -E python3 main.py
import os

DEVICE_ID = os.environ.get("ASTRA_DEVICE_ID", "helios-001")


def _topico(sufixo):
    return f"astra/{DEVICE_ID}/{sufixo}"


# ================= MQTT =================
# 'localhost' porque o broker Mosquitto roda na própria Orange Pi -
# assim não quebra se o IP da rede mudar.
MQTT_BROKER = "localhost"
MQTT_PORT = 1883
MQTT_CLIENT_ID = f"OrangePi_ASTRA_{DEVICE_ID}"

TOPICO_STATUS       = _topico("status")
TOPICO_MANHA        = _topico("manha")
TOPICO_NOITE        = _topico("noite")
TOPICO_ALARME       = _topico("alarme")
TOPICO_CMD          = _topico("cmd")
TOPICO_HORARIO_MANHA = _topico("horario/manha")
TOPICO_HORARIO_NOITE = _topico("horario/noite")
TOPICO_SIMON        = _topico("simon")

# Tópicos usados pelo dashboard web pra editar paciente/medicamentos.
# O Node.js só publica nesses tópicos - quem grava no banco é sempre
# o main.py (Python), ao receber a mensagem.
TOPICO_PACIENTE            = _topico("paciente/nome")
TOPICO_MEDICAMENTO_MANHA   = _topico("medicamento/manha")
TOPICO_MEDICAMENTO_NOITE   = _topico("medicamento/noite")

# ================= ALARMES (padrão) =================
HORA_MANHA_PADRAO = 9
MIN_MANHA_PADRAO  = 0
HORA_NOITE_PADRAO = 22
MIN_NOITE_PADRAO  = 0

# ================= VERIFICAÇÃO DE ADERÊNCIA (jogo tipo Simon) =================
# Depois de confirmar uma dose real (botão verde), espera esse tempo e faz
# um teste de responsividade: pisca os LEDs numa sequência aleatória e a
# pessoa precisa repetir apertando os botões correspondentes. Se errar ou
# não responder a tempo, repete a MESMA sequência de novo - cada tentativa
# (certa ou errada) fica registrada no banco.
SIMON_ATRASO_SEG = 3 * 60        # espera 3 minutos após confirmar
SIMON_TIMEOUT_PASSO_SEG = 20      # tempo pra apertar cada botão antes de "não respondeu"
SIMON_DURACAO_FLASH_SEG = 0.6     # quanto tempo cada LED fica aceso na demonstração
SIMON_PAUSA_FLASH_SEG = 0.35      # pausa apagada entre um LED e o próximo
SIMON_CORES = ("verde", "amarelo", "azul")  # cores em jogo (usa os LED/BTN _VERDE/_AMARELO/_AZUL)

# Modo FIXO (padrão se SIMON_MODO_INCREMENTAL = False): sempre uma
# sequência de SIMON_PASSOS, uma rodada só.
SIMON_PASSOS = 3

# Modo INCREMENTAL (estilo Simon clássico): começa com
# SIMON_PASSOS_INICIAL e, a cada acerto da rodada inteira, soma +1 na
# sequência e repete tudo de novo desde o início - até chegar em
# SIMON_PASSOS_MAXIMO, quando aí sim a verificação é dada como concluída.
# Um erro/timeout não reseta o progresso: repete só a rodada atual.
SIMON_MODO_INCREMENTAL = True
SIMON_PASSOS_INICIAL = 1
SIMON_PASSOS_MAXIMO = 5

# ================= ALERTA DE NÃO ADERÊNCIA =================
# Se o alarme ficar ativo (sem confirmação) por esse tempo, entra no
# modo de alerta: LED vermelho piscando + beep curto periódico.
TEMPO_NAO_ADERENCIA_SEG = 5 * 60  # 5 minutos

# Intervalo entre os beeps curtos durante o modo de não aderência.
INTERVALO_BUZZER_NAO_ADERENCIA_SEG = 90

# Duração de cada beep curto nesse modo.
DURACAO_BEEP_NAO_ADERENCIA_SEG = 1.5

# ================= ARQUIVO DE ESTADO =================
# Substitui o Preferences/NVS do ESP32.
ARQUIVO_ESTADO = "/home/orangepi/Projetos/astra_estado.json"

# Banco SQLite: paciente, medicamentos (editáveis pelo dashboard) e
# histórico de doses. O dashboard web lê esse arquivo diretamente.
DB_PATH = "/home/orangepi/Projetos/astra.db"

# ================= TIMEZONE =================
# A Orange Pi deve estar com o fuso horário correto do sistema.
# Rode uma vez: sudo timedatectl set-timezone America/Sao_Paulo
