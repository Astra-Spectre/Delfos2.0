...
type SimonFase    = "exibindo" | "aguardando" | null;
type SimonResultado = "OK" | "ERRO_BOTAO" | "TIMEOUT" | null;

interface SimonEstado {
  fase: SimonFase;
  tentativa: number;
  ultimo_resultado: SimonResultado;
  total_passos?: number;
}
...
interface DoseLog {
  ...
  status:              string;
  simon_tentativas:     number;
  simon_ultimo_resultado: SimonResultado;
}
...
function SimonResultCell({ tentativas, resultado }: { tentativas: number; resultado: "OK" | "ERRO_BOTAO" | "TIMEOUT" | null }) {
  if (!tentativas) {
    return <span style={{ fontFamily: C.mono, fontSize: "0.68rem", color: C.textMuted }}>—</span>;
  }

  const passou = resultado === "OK";
  const cor = passou ? C.okText : C.warnText;
  const bg  = passou ? C.okBg   : C.warnBg;
  const borda = passou ? C.okBorder : C.warnBorder;
  const rotulo = passou
    ? (tentativas > 1 ? `✓ OK (${tentativas}x)` : "✓ OK")
    : `⚠ tentativa ${tentativas}`;

  return (
    <span style={{
      fontFamily: C.mono, fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.05em",
      padding: "3px 8px", borderRadius: "3px",
      background: bg, color: cor, border: `1px solid ${borda}`,
    }} title={
      resultado === "TIMEOUT" ? "Não respondeu a tempo em pelo menos uma tentativa"
      : resultado === "ERRO_BOTAO" ? "Apertou o botão errado em pelo menos uma tentativa"
      : "Verificação de responsividade concluída"
    }>
      {rotulo}
    </span>
  );
...
function LegendaCor({ cor, label }: { cor: string; label: string }) {
  return (
    <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
      <span style={{ width: "8px", height: "8px", borderRadius: "2px", background: cor, display: "inline-block" }} />
      {label}
    </span>
  );
}
...
function CalendarioMes({
  mes, dias, loading, selecionado, onSelecionar,
}: {
  mes: string;
  dias: Record<string, ResumoDia>;
  loading: boolean;
  ...
  for (let dia = 1; dia <= diasNoMes; dia++) celulas.push(`${anoStr}-${mesStr}-${pad2(dia)}`);
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: "4px", marginBottom: "4px" }}>
        {["D", "S", "T", "Q", "Q", "S", "S"].map((d, i) => (
          <div key={i} style={{ textAlign: "center", fontFamily: C.mono, fontSize: "0.55rem", color: C.textMuted, letterSpacing: "0.1em" }}>
            {d}
          </div>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: "4px", opacity: loading ? 0.4 : 1, transition: "opacity 0.15s" }}>
        {celulas.map((dataStr, i) => {
          if (!dataStr) return <div key={`vazio-${i}`} />;
          const resumo = dias[dataStr];
          const diaNum = Number(dataStr.slice(-2));
          const isHoje = dataStr === hojeStr;
          const isSelecionado = dataStr === selecionado;

          let corFundo = "transparent";
          let corBorda = C.cardBorder;
          let corTexto: string = resumo ? C.textSub : C.textMuted;

          if (resumo) {
            if (resumo.perdido > 0)       { corFundo = C.dangerBg; corBorda = C.dangerBorder; corTexto = C.dangerText; }
            else if (resumo.pendente > 0) { corFundo = C.warnBg;   corBorda = C.warnBorder;   corTexto = C.warnText; }
            else if (resumo.ok > 0)       { corFundo = C.okBg;     corBorda = C.okBorder;     corTexto = C.okText; }
          }

          return (
            <button
              key={dataStr}
              onClick={() => resumo && onSelecionar(dataStr)}
              disabled={!resumo}
              title={resumo ? `${resumo.ok}/${resumo.total} confirmadas` : "Sem registros"}
              style={{
                aspectRatio: "1", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                borderRadius: "6px", border: `1px solid ${isSelecionado ? corTexto : corBorda}`,
                background: corFundo,
                cursor: resumo ? "pointer" : "default",
                fontFamily: C.mono, fontSize: "0.68rem", color: corTexto,
                outline: isHoje ? `1px solid ${C.text}` : "none",
                outlineOffset: "-2px",
                opacity: resumo ? 1 : 0.35,
                transition: "transform 0.1s",
                padding: 0,
              }}
              onMouseEnter={e => { if (resumo) e.currentTarget.style.transform = "scale(1.06)"; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; }}
            >
              <span style={{ fontWeight: 700 }}>{diaNum}</span>
              {resumo && (
                <span style={{ fontSize: "0.5rem", opacity: 0.85 }}>{resumo.ok}/{resumo.total}</span>
              )}
            </button>
          );
        })}
      </div>
      <div style={{ display: "flex", gap: "14px", marginTop: "12px", fontFamily: C.mono, fontSize: "0.58rem", color: C.textMuted }}>
        <LegendaCor cor={C.okText} label="Tudo OK" />
        <LegendaCor cor={C.warnText} label="Pendente" />
        <LegendaCor cor={C.dangerText} label="Perdido" />
      </div>
    </div>
  );
}
...
export default function App() {
  const [estado,         setEstado]         = useState<Estado>({
    alarme: "OFF", manha: "PENDENTE", noite: "PENDENTE", status: "OFFLINE",
  ...
  const [diasCalendario, setDiasCalendario] = useState<Record<string, ResumoDia>>({});
  ...
  const carregarHistorico = useCallback((dataFiltro?: string) => {
    setHistLoading(true);
    const url = dataFiltro ? `/api/historico?data=${dataFiltro}&limite=50` : "/api/historico?limite=30";
    fetch(url)
      .then(r => r.json())
      .then(d => {
  ...
  const carregarCalendario = useCallback((mes: string) => {
    setCalLoading(true);
    fetch(`/api/calendario?mes=${mes}`)
      .then(r => r.json())
      .then(d => {
        if (!d.ok) { showToast("Falha ao carregar calendário"); return; }
        const mapa: Record<string, ResumoDia> = {};
        (d.dias as ResumoDia[]).forEach(dia => { mapa[dia.data] = dia; });
        setDiasCalendario(mapa);
      })
      .catch(() => showToast("Falha ao carregar calendário"))
      .finally(() => setCalLoading(false));
  }, [showToast]);

  // Carrega config + calendário do mês atual, ao montar (o estado em tempo
  // real já vem separado, via WebSocket). O histórico detalhado só carrega
  // quando a pessoa clica num dia do calendário.
  useEffect(() => {
    carregarConfig();
    carregarCalendario(mesAtual);
  }, [carregarConfig, carregarCalendario, mesAtual]);
  ...
  function selecionarDia(dataStr: string) {
    ...
    carregarHistorico(dataStr);
  }

  function mudarMes(delta: number) {
    const [ano, mes] = mesAtual.split("-").map(Number);
    const d = new Date(ano, mes - 1 + delta, 1);
    setMesAtual(`${d.getFullYear()}-${pad2(d.getMonth() + 1)}`);
    setDiaSelecionado(null);
    setHistorico([]);
  }
  ...
  function reloadHistorico() {
    if (diaSelecionado) {
      carregarHistorico(diaSelecionado);
    } else {
      carregarCalendario(mesAtual);
    }
        ...
        <div style={{ padding: "20px 20px 60px", maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <div className="grid-main" style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              <Panel accent>
                ...
                <CmdBtn variant="ghost" onClick={() => handleCmd("TESTE_SIMON")} disabled={cmdLoading === "TESTE_SIMON" || estado.simon.fase !== null}>
                  🎮 &nbsp;Testar jogo Simon
                </CmdBtn>
                ...
                {estado.simon.fase !== null && (
                  <div style={{
                    marginTop: "10px", padding: "10px 12px", borderRadius: "8px",
                    background: C.warnBg, border: `1px solid ${C.warnBorder}`,
                    fontFamily: C.mono, fontSize: "12px", color: C.warnText,
                  }}>
                    🎮 Verificação em andamento
                    {estado.simon.total_passos ? ` — rodada de ${estado.simon.total_passos} passo${estado.simon.total_passos > 1 ? "s" : ""}` : ""}
                    {" "}— tentativa {estado.simon.tentativa} —{" "}
                    {estado.simon.fase === "exibindo" ? "mostrando sequência..." : "aguardando resposta..."}
                  </div>
                )}
              </Panel>
          ...
          <Panel style={{ marginTop: "14px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
              ...
              <CmdBtn variant="refresh" onClick={reloadHistorico} disabled={histLoading || calLoading}>
                {(histLoading || calLoading) ? "Atualizando..." : "↻ Atualizar"}
              </CmdBtn>
            </div>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
              <button onClick={() => mudarMes(-1)} style={{
                background: "transparent", border: `1px solid ${C.cardBorder}`, borderRadius: "6px",
                color: C.textSub, width: "26px", height: "26px", cursor: "pointer", fontSize: "0.9rem",
              }}>‹</button>
              <span style={{ fontFamily: C.mono, fontSize: "0.72rem", color: C.text, letterSpacing: "0.1em", textTransform: "uppercase" }}>
                {nomeMes(mesAtual)}
              </span>
              <button onClick={() => mudarMes(1)} style={{
                background: "transparent", border: `1px solid ${C.cardBorder}`, borderRadius: "6px",
            ...
            {diaSelecionado && (
              <div style={{ marginTop: "18px", borderTop: `1px solid ${C.cardBorder}`, paddingTop: "14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                  <span style={{ fontFamily: C.mono, fontSize: "0.68rem", color: C.textSub, letterSpacing: "0.05em" }}>
                    Detalhes de {diaSelecionado}
                  </span>
                  <button onClick={() => selecionarDia(diaSelecionado)} style={{
                    background: "transparent", border: "none", color: C.textMuted,
                    cursor: "pointer", fontFamily: C.mono, fontSize: "0.65rem",
                  }}>✕ fechar</button>
                </div>
                ...
                {histLoading ? (
                  ...
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: C.mono, fontSize: "0.72rem" }}>
                      <thead>
                        <tr>
                          {["Data", "Período", "Medicamento", "Programado", "Tomado", "Status", "Verificação"].map(h => (
                            <th key={h} style={{
                      ...
                      <tbody>
                        {historico.map((h, i) => (
                          ...
                          >
                            ...
                            <td style={{ padding: "9px 12px" }}>
                              <SimonResultCell tentativas={h.simon_tentativas} resultado={h.simon_ultimo_resultado} />
                            </td>
                          </tr>
                        ))}
                      </tbody>
