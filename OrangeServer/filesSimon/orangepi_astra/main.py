...
class AstraController:
    def __init__(self):
        ...
        self.mqtt = AstraMQTT(
            ...
            on_simon_status=self.on_simon_status,
        )
    ...
    def on_simon_status(self, status):
        """Callback para status do jogo Simon (MQTT)"""
    ...
    def _iniciar_verificacao_simon(self, agora_monot):
        if not self.simon_sequencia:
            self.simon_sequencia = [
                random.choice(config.SIMON_CORES) for _ in range(config.SIMON_PASSOS)
            ]
            print(f"[simon] Nova verificação - sequência: {self.simon_sequencia}")
        else:
            print(f"[simon] Tentativa {self.simon_tentativa} - repetindo a mesma sequência")
    ...
    def _marcar_tomado(self):
        agora = datetime.now()
        periodo = self.periodo_atual or ("manha" if agora.hour < 15 else "noite")
        ...
        if self.periodo_atual is not None:
            dose_id = database.confirmar_dose(self.periodo_atual, agora.strftime("%Y-%m-%d"))
    ...
    def _publicar(self, simon_status=""):
        self.mqtt.publicar_estado(
            self.remedio_manha_tomado, self.remedio_noite_tomado, self.alarme_ativo, simon_status
        )
