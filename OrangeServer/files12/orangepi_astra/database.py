...
def registrar_disparo(periodo, paciente_nome, medicamento_nome, horario_programado):
    ...
    linha PENDENTE no histórico e retorna o id dela."""
    agora = datetime.now()
    conn = _conectar()
    cursor = conn.execute(
...
def confirmar_dose_por_id(dose_id):
    """Marca uma dose específica (pelo id, capturado no momento do disparo
    do alarme) como tomada - não importa o status atual dela. Isso permite
    corrigir uma dose que a virada do dia já tinha marcado como PERDIDO,
    caso o usuário confirme depois da meia-noite."""
    if dose_id is None:
        return None
    agora = datetime.now()
    conn = _conectar()
...
def confirmar_dose(periodo, data):
    ...
    Mantido para compatibilidade - prefira confirmar_dose_por_id quando
    o id da dose já é conhecido (evita o problema de vira-dia)."""
    agora = datetime.now()
    conn = _conectar()
    row = conn.execute(
...
def dose_tomada_hoje(periodo, data):
    """True se já existe alguma dose desse período, nessa data, com
    status OK. É a fonte única de verdade sobre 'já tomou ou não' -
    usada tanto ao iniciar o sistema quanto ao rearmar, pra nunca
    reverter uma confirmação real só porque alguém apertou 'rearmar'."""
    conn = _conectar()
    row = conn.execute(
        "SELECT 1 FROM doses_log WHERE periodo = ? AND data = ? AND status = 'OK' LIMIT 1",
        (periodo, data),
    ).fetchone()
    conn.close()
