...
class AstraController:
    ...
    def _marcar_tomado(self):
        ...
        # só loga no histórico se foi um alarme real (não um teste manual)
        if self.periodo_atual is not None:
            database.confirmar_dose(self.periodo_atual, agora.strftime("%Y-%m-%d"))
...
def main():
    ...
    while True:
        ...
        if agora_monot - ultimo_display >= 0.2:
            ...
            if controlador.em_nao_aderencia(agora_monot):
